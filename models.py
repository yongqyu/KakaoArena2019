...
class GMF(torch.nn.Module):
    ...
    def forward(self, data):#reader_indices, item_indices, item_keywd, negs_indices, negs_keywd):
        ...
        #element_product = torch.mul(reader_embedding, torch.cat([writer_embedding, item_kw_embedding], 1))
        #pos_logits = self.affine_output(element_product)
        pos_logits = torch.mm(reader_embedding, item_joint_embedding.t())
        ...
        #element_product = torch.mul(reader_embedding, torch.cat([negs_embedding, negs_kw_embedding], 1))
        #neg_logits = self.affine_output(element_product)
        neg_logits = torch.mm(reader_embedding, negs_joint_embedding.t())
    ...
    def predict(self, reader, top=100):
        ...
        #total_writer_embedding = self.embedding_writer.weight
        ...
        #item_joint_embedding = torch.cat([item_embedding, writer_embedding, keywd_output[:,-1,:]], 1)
        item_joint_embedding = self.item_fc(torch.cat([item_embedding, writer_embedding, keywd_output[:,-1,:]], 1))
        dot_product = torch.mm(reader_embedding, item_joint_embedding.t())
